...
// ============== SUPABASE SETUP ==============
// ดึงค่าเชื่อมต่อจากไฟล์ .env (สำหรับ Vite)
...
// ใช้รูปโลโก้ของคุณ (นำไฟล์ S__22413315.jpg ไปใส่ในโฟลเดอร์ public/ ใน VS Code)
...
const INITIAL_DATA = {
  ...
  policies: [
    {policy_id:"POL-001", policy_no:"1", category:"นโยบายหลัก", audience:"-", meeting:"-", timeframe:"ปีงบประมาณ 2569", commander:"ผบ.ทสส.", order:"ยก.ทหาร ประสาน กบ.ทหาร ถอดบทเรียนรวมทั้งปัญหาข้อขัดข้องต่าง ๆ ที่เกิดขึ้นจากกรณีการจัดหายานเกาะล้อยางเพื่อทดแทน V-150 ในการสนับสนุนปฏิบัติภารกิจรักษาสันติภาพเพื่อนำมาให้ดำเนินการในโอกาสต่อไป", primary_unit:"กกล.กบ.ทหาร", secondary_units:[]},
    {policy_id:"POL-002", policy_no:"2", category:"สั่งการเพิ่มเติม", audience:"หน.ส่วนราชการ", meeting:"ครั้งที่ 1", timeframe:"ดำเนินการทันที", commander:"ผบ.ทสส.", order:"กำกับดูแลการปฏิบัติการจัดซื้อจัดจ้างให้เป็นไปตามแผนงานและกรอบระยะเวลาที่กำหนดเพื่อไม่ให้เกิดความเสียหาย และต้องเป็นประโยชน์ต่อองค์กรหลัก", primary_unit:"กคง.กบ.ทหาร", secondary_units:[]},
    {policy_id:"POL-003", policy_no:"3", category:"สั่งการเพิ่มเติม", audience:"หน.ส่วนราชการ", meeting:"ครั้งที่ 1", timeframe:"ปีงบประมาณ 2569", commander:"ผบ.ทสส.", order:"กบ.ทหาร ประสานส่วนราชการที่เป็นหน่วยเจ้าของงาน/โครงการ ของ บก.ทท. ประจำปีงบประมาณ พ.ศ. 2569 และหน่วยที่ดำเนินกรรมวิธีจัดซื้อจัดจ้าง ต้องดำเนินการอย่างถูกต้อง อย่าให้เกิดปัญหา", primary_unit:"กนผ.สสร.กบ.ทหาร", secondary_units:[]},
    {policy_id:"POL-010", policy_no:"10", category:"สั่งการเพิ่มเติม", audience:"กรม สธร.ฯ", meeting:"ครั้งที่ 1", timeframe:"10-13 พ.ย. 68", commander:"เสธ.ทหาร", order:"ทุกส่วนราชการ ให้ความสำคัญกับการเยี่ยมชมงานนิทรรศการอุปกรณ์ป้องกันประเทศ ดีเฟนส์แอนด์ซิคิวริตี้ 2025 ใน 10-13 พ.ย. 68 เพื่อศึกษาข้อมูล และนำกลับมาพัฒนากองทัพต่อไป", primary_unit:"กยป.สสร.กบ.ทหาร", secondary_units:[]}
  ]
};
    ...
    <div className="bg-slate-900 text-slate-100 min-h-screen flex font-sans">
      <style>{`
        ...
        @media print {
          ...
          .bg-slate-900, .bg-slate-800, .bg-amber-900\\/40, .bg-red-900\\/20, .bg-sky-900\\/20, .bg-slate-900\\/50 { background-color: #ffffff !important; color: #000000 !important; border: 1px solid #ccc !important; box-shadow: none !important; }
          .text-slate-100, .text-slate-200, .text-slate-300, .text-slate-400, .text-white { color: #111827 !important; }
          .text-amber-400, .text-amber-500 { color: #b45309 !important; font-weight: bold; }
          ...
          .text-sky-400, .text-sky-500 { color: #0284c7 !important; font-weight: bold; }
    ...
    <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
      <div className="bg-slate-800 p-8 rounded-2xl shadow-2xl w-full max-w-md border border-slate-700 fade-in-up relative overflow-hidden">
        ...
        <div className="text-center mb-8 relative z-10">
          ...
          {!supabase && <p className="text-red-400 text-xs mt-2 bg-red-400/10 p-1 rounded">กำลังทำงานในโหมดจำลอง (ตรวจสอบไฟล์ .env)</p>}
        </div>
...
function PolicyDashboard({ appDb, user, showToast, refresh }) {
  ...
  const stats = useMemo(() => {
    ...
    if (filterUnit !== 'ALL') {
      fPolicies = fPolicies.filter(p => p.primary_unit === filterUnit || p.secondary_units?.includes(filterUnit) || p.primary_unit === 'ทุกหน่วย');
    }

    let fReports = appDb.reports;

    if (filterUnit !== 'ALL') {
      fReports = fReports.filter(r => r.unit_name === filterUnit);
    }
    if (filterStart) fReports = fReports.filter(r => new Date(r.report_date) >= new Date(filterStart));
    if (filterEnd) {
      const end = new Date(filterEnd);
      end.setHours(23, 59, 59, 999);
      fReports = fReports.filter(r => new Date(r.report_date) <= end);
    }

    const progList = fPolicies.map(po => {
      const rs = fReports.filter(r => r.policy_id === po.policy_id && (filterUnit === 'ALL' || r.unit_name === filterUnit))
                         .sort((a, b) => new Date(b.report_date) - new Date(a.report_date));
      return { 
        id: po.policy_id, 
        name: po.order, 
        short: `[ลำดับ ${po.policy_no || '-'}] ${po.order.length > 50 ? po.order.substring(0, 50) + '...' : po.order}`, 
        progress: rs.length ? rs[0].progress_percent : 0 
      };
    });
  ...
  const handleSyncInitialData = async () => {
    if (!supabase) return showToast('ยังไม่ได้เชื่อมต่อ Supabase', 'error');
    if (window.confirm("ระบบจะทำการสร้างข้อมูล 'หน่วยงาน' และ 'ข้อสั่งการ' ตั้งต้น คุณแน่ใจหรือไม่? (ข้อมูลเก่าจะถูกทับ)")) {
    ...
    <div className="space-y-6 fade-in-up">
      {isDbEmpty && user.role === 'admin' && (
        <div className="bg-amber-900/40 border border-amber-500 p-6 rounded-xl flex flex-col md:flex-row items-center justify-between gap-4 print-hide">
          <div>
            ...
            <p className="text-slate-300 text-sm mt-1">กดปุ่มด้านขวาเพื่อนำรายชื่อ 11 หน่วยงาน และ 14 ข้อสั่งการตั้งต้นเข้าสู่ระบบ</p>
          </div>
      ...
      <div className="bg-slate-800 p-5 rounded-xl border border-slate-700 print-hide">
        <div className="flex items-center gap-2 mb-4 text-amber-400">
          ...
          <h3 className="font-semibold text-lg">ภาพรวมข้อสั่งการและนโยบาย</h3>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-slate-800 p-6 rounded-xl border-l-4 border-blue-500 shadow-md">
          <p className="text-slate-400 text-sm font-medium">จำนวนข้อสั่งการ</p>
        ...
        <div className="bg-slate-800 p-6 rounded-xl border-l-4 border-yellow-500 shadow-md">
          <p className="text-slate-400 text-sm font-medium">การรายงานความคืบหน้า</p>
          <h3 className="text-3xl font-bold text-white mt-1">{stats.totalReports}</h3>
          <p className="text-xs text-slate-500 mt-2">ฉบับ</p>
        </div>
        <div className="bg-slate-800 p-6 rounded-xl border-l-4 border-amber-500 shadow-md">
          <p className="text-slate-400 text-sm font-medium">ภาพรวมความคืบหน้า</p>
...
function TaskDashboard({ appDb, user }) {
  ...
  const stats = useMemo(() => {
    ...
    return { totalTasks, completedTasks, delayedTasks, avgProgress, progList, statusCount };
  }, [appDb, filterUnit, filterStart, filterEnd]);

  let cumulativePercent = 0;
  const donutGradientStops = stats.statusCount.length > 0 ? stats.statusCount.map(d => {
    const start = cumulativePercent;
    const slicePercent = (d.value / stats.totalTasks) * 100;
    cumulativePercent += slicePercent;
  ...
  return (
    <div className="space-y-6 fade-in-up">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-4 bg-slate-800 p-6 rounded-xl border border-slate-700 shadow-md flex flex-col items-center">
          <h3 className="text-lg font-semibold mb-6 flex items-center gap-2 w-full"><Activity size={20} className="text-slate-400"/> สถานะการดำเนินงาน (Status)</h3>
          
          <div className="relative w-48 h-48 rounded-full mb-6" style={{ background: stats.totalTasks > 0 ? `conic-gradient(${donutGradientStops})` : '#1e293b' }}>
            <div className="absolute inset-0 m-auto w-32 h-32 bg-slate-800 rounded-full flex items-center justify-center border-[8px] border-slate-800">
              <div className="text-center">
                <div className="text-2xl font-bold">{stats.totalTasks}</div>
                <div className="text-[10px] text-slate-400">งานทั้งหมด</div>
              </div>
            </div>
          </div>
          ...
          <div className="w-full space-y-2 mt-auto">
            ...
            {stats.totalTasks === 0 && <p className="text-center text-sm text-slate-500">ไม่มีข้อมูลภารกิจ</p>}
          </div>
        </div>

        <div className="lg:col-span-8 bg-slate-800 p-6 rounded-xl border border-slate-700 shadow-md flex flex-col">
          <h3 className="text-lg font-semibold mb-6 flex items-center gap-2"><BarChart size={20} className="text-slate-400"/> ความคืบหน้าของงานแต่ละรายการ</h3>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-5" style={{ maxHeight: '350px' }}>
            ...
            {stats.progList.length === 0 && <p className="text-center text-sm text-slate-500 mt-10">ไม่มีข้อมูลภารกิจ</p>}
          </div>
        </div>
      </div>
    </div>
